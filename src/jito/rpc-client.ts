...
import { JITO_BUNDLES_URL, JITO_RPC_URL } from "@/config";
import type {
  GetBundleStatusesParams,
  GetBundleStatusesResult,
  GetInflightBundleStatusesParams,
  GetInflightBundleStatusesResult,
  GetTipAccountsResult,
  GetTipFloorResult,
  RpcResponse,
  SendBundleParams,
  SendBundleResult,
...
class JitoJsonRpcClient {
  ...
  private uuid;
  private client;
  private logger?: typeof console; // ✅ Fixed: was Console, now typeof console
  ...
  async getInFlightBundleStatuses(
    params: GetInflightBundleStatusesParams
  ): Promise<RpcResponse<GetInflightBundleStatusesResult>> {
    const endpoint = this.uuid ? `/bundles?uuid=${this.uuid}` : "/bundles";
  ...
  async confirmInflightBundle(bundleId: string, timeoutMs = 60000) {
    ...
    while (Date.now() - start < timeoutMs) {
      try {
        ...
        if (response.result?.value && response.result.value.length > 0) {
          ...
          if (bundleStatus.status === "Failed") {
            ...
            if (detailedStatus.result?.value && detailedStatus.result.value.length > 0) {
  ...
  async getTipFloor() {
    const response = await axios.get<GetTipFloorResult>(`${JITO_BUNDLES_URL}/api/v1/bundles/tip_floor`);
    return response.data;
  }
}

export const jitoClient = new JitoJsonRpcClient(`${JITO_RPC_URL}/api/v1`);
