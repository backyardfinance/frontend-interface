...
class JitoJsonRpcClient {
  ...
  async getTipFloor() {
    const baseUrl = import.meta.env.VITE_PUBLIC_JITO_BUNDLES_URL ?? "https://bundles.jito.wtf";

    const response = await axios.get<GetTipFloorResult>(`${baseUrl}/api/v1/bundles/tip_floor`);
    return response.data;
  }
}
