...
export const TopVaults: React.FC<Props> = ({ vaults, onAdd }) => {
      ...
      {vaults?.map((vault: Vault) => (
        ...
        >
          <div className="flex flex-row items-center justify-center gap-[7px] rounded-2xl bg-white p-[8px] outline outline-zinc-100 outline-offset-[-1px]">
            ...
            <button
              className="flex h-8 w-7 rotate-90 items-center justify-center rounded-lg bg-stone-50 p-2 outline outline-zinc-100 outline-offset-[-1px]"
              onClick={(e) => onAdd(e, vault)}
              type="button"
            >
